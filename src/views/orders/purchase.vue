<template>
	<div class="body-container">
		<a-page-header class="demo-page-header" :title="data.lang == 'zh' ? '采购单' : 'Purchase Order'" @back="() => $router.go(-1)">
			<template #extra>
				<a-button key="3" @click="downloadAllDrawings">{{ data.lang == 'zh' ? '下载全部图纸' : 'Download All Drawings' }}</a-button>
				<a-button key="1" :disabled="data.main.supply.id == undefined" :loading="data.main.processLoading" @click="saveQuote">生成文档</a-button>
				<a-button key="2" type="primary" :disabled="data.main.pdfPath == undefined" :loading="data.main.processOrderBtnLoading" @click="makeProcessOrder">
					进入生产流程
				</a-button>
			</template>
		</a-page-header>
			...
			<div class="review-wrap">
				<div class="html-container" id="content">
					...
					<div class="sub-header">
						<div class="flex">
							<div class="left">
								<div class="item">{{ data.lang == 'zh' ? '甲（需）方' : 'Party A' }}: 深圳市波士谭管理科技有限公司</div>
							</div>
							<div class="right">
								<div class="item">
									{{ data.lang == 'zh' ? '乙（供）方' : 'Party B' }}:
									<a v-if="data.main.supply.id == undefined" class="link" style="color: #ff0000" @click="showSupplyMokdal">
										{{ data.lang == 'zh' ? '请选择供应商' : 'Please select a supplier' }}
									</a>
									<a class="link" style="color: #444" @click="showSupplyMokdal">{{ data.main.supply.name }}</a>
								</div>
							</div>
						</div>
						<div style="margin: 10px; border-top: 1px solid #e1e1e1"></div>
						<div class="flex" v-if="data.main.orderInfo">
							<div class="left">
								<div class="item" style="color: #1b8bf5">{{ data.lang == 'zh' ? '收货信息' : 'Shipping' }}</div>
								...
								<div class="item">
									{{ data.lang == 'zh' ? '收货地址' : 'Address' }}：{{ data.main.orderInfo.country_text }} {{ data.main.orderInfo.state_text }}
									{{ data.main.orderInfo.city }} {{ data.main.orderInfo.address }}
								</div>
							</div>
							<div class="right">
					...
					<table class="mk-table">
						...
						<tbody>
							...
							<tr v-for="(item, index) in data.main.source" :key="index">
								...
								<td>
									<p class="mg8">重量：{{ (item.goods_data.weight * 1).toFixed(2) }}g</p>
									<p class="mg8">尺寸：{{ item.goods_data.width }}x{{ item.goods_data.height }}x{{ item.goods_data.length }}mm</p>
								...
								<td style="font-size: 13px">
									{{ item.goods_data.configure?.notes ? item.goods_data.configure?.notes : '-' }}
								</td>
								<td>{{ item.quantity }}</td>
								<td>￥{{ (item.goods_data.weight * 0.3).toFixed(2) }}</td>
								<!-- <td style="text-align: center">￥{{ (item.unit_price * 1).toFixed(2) }}</td> -->
								<td>￥{{ (item.quantity * (item.goods_data.weight * 0.3)).toFixed(2) }}</td>
							</tr>
					...
					<div class="footer">
						<div class="flex" style="justify-content: space-between; align-items: baseline">
							<section class="company-section">
								<div class="item">甲（需）方：深圳市波士谭管理科技有限公司</div>
							...
							<section class="company-section">
								<div class="item">乙（供）方：{{ data.main.supply.name }}</div>
								<div class="item">联系人：{{ data.main.supply.contact }}</div>
								<div class="item">联系电话：{{ data.main.supply.phone }}</div>
								<div class="item">乙方审核盖章：</div>
							</section>
						</div>
					</div>
				</div>
...
<script setup>
	...
	import { getAdminOrderInfoApi, getOrderPackageApi } from '@/api/order'
	...
	const data = ref({
		...
		main: {
			...
			downloadAllDrawingsLink: undefined,
			supplySource: [],
			supply: {
				id: undefined,
				name: undefined,
	...
	const downloadAllDrawings = () => {
		data.value.main.downloadLoading = true
		if (data.value.main.downloadAllDrawingsLink) {
			window.open(data.value.main.downloadAllDrawingsLink)
			data.value.main.downloadLoading = false
			return
		}
		getOrderPackageApi({ order_no: data.value.main.orderNo })
			.then((res) => {
				data.value.main.downloadAllDrawingsLink = res.data.download_url
				window.open(data.value.main.downloadAllDrawingsLink)
			})
			.finally(() => {
				data.value.main.downloadLoading = false
	...
	const getOrderInfo = async () => {
		...
		if (response.code === 0) {
			...
			for (const item of items) {
				if (item.goods_data.preview) {
					item.loading = true
					try {
						item.preview_base64 = await getUrlBase64(item.goods_data.preview)
					} catch (e) {
						item.preview_base64 = ''
					}
					item.loading = false
			...
			if (response.data.quote.technology == 1) {
				data.value.main.quotationTitle = window.lang == 'en' ? '3D Print Quotation' : '3D打印采购单'
			} else if (response.data.quote.technology == 2) {
				data.value.main.quotationTitle = window.lang == 'en' ? 'CNC Quotation' : 'CNC机加工采购单'
			}

			data.value.main.userInfo = response.data.user
			data.value.main.orderInfo = response.data
			data.value.main.firstTotal = items.map((item) => parseFloat(item.goods_data.weight * data.value.supplyPrice)).reduce((a, b) => a + b, 0)
			data.value.main.total = items.map((item) => parseFloat(item.quantity * (item.goods_data.weight * data.value.supplyPrice))).reduce((a, b) => a + b, 0)
		}
	...
	const makeProcessOrder = () => {
		data.value.main.processOrderBtnLoading = true
		processOrderApi({ order_no: data.value.main.orderNo, path: data.value.main.pdfPath, status: 5, lang: data.value.lang, type: 0 })
			.then((result) => {
				if (result.code == 0) {
	...
	const getUrlBase64 = (url) => {
		return new Promise((resolve) => {
			...
			// 关键逻辑：图片 404 或加载失败时的兼容处理
			img.onerror = function () {
				console.warn('图片加载失败，已替换为占位图:', url)
				// 这里返回一个 1x1 像素的透明图片 Base64
				// 或者你可以放一个项目里的本地默认图路径
